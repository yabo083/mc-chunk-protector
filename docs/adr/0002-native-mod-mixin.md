...
- 模式 B（冻结更新）：**Mixin `@Inject` 短路 `net.minecraft.server.level.ServerLevel#updateNeighborsAt`**（及 `Level#updateNeighborsAt`），当被更新方块的区块在冻结区时直接 return，不向 6 个邻居广播。
- 构建用 **javac 免 Gradle**（classpath 指向 dev-server 已装 Neoforge libraries），`mod/build.ps1`。
- 配置仍读 `kubejs/config/regions.json`（与 GUI 共契约）。

## 后果
- 正面：Mixin 在正确层生效，冻结可感知（栅栏放置后不连接邻居）；不依赖 KubeJS；仅服务端逻辑，客户端无副作用。
- 负面 / 代价：需维护 Java 工程与 mixin 描述符；修改 `Level` vanilla 方法有一定 ABI 风险（1.21.1 方法名已实测匹配 `updateNeighborsAt(Lnet/minecraft/core/BlockPos;Lnet/minecraft/world/level/block/Block;)V`）。
